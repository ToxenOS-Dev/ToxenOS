...
void _start(void) {
    ...
    if (!src[0] || !dst[0]) {
        put_err("move: usage: move <source> <dest>", ""); sys_exit(1);
    }
    ...
    if (my_strlen(idst) > slen && idst[slen] == '/' &&
        ...
        put_err("move: cannot move a folder into itself: ", src); sys_exit(1);
    }

    int64_t r = sys_rename(isrc, idst);
    if (r == SYS64_ERR_PROTECTED) { put_err("move: protected path: ", src); sys_exit(1); }
    if (r == SYS64_ERR_EXISTS)    { put_err("move: destination already exists: ", dst); sys_exit(1); }
    if (r < 0)                    { put_err("move: cannot move: ", src); sys_exit(1); }
    sys_exit(0);
    for (;;) {}
}
