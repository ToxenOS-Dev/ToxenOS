...
user:
	...
	for cmd in ls shw mkef mkd rm echo pcd uname file help cp tree hex mv rname sif find bmsg proc end top sleeptest memtest pipetest nettest dns http ping https isolation_test stresstest restore trash rmkd sysctl kill reg syslog wc date chmod where df free hostname adduser passwd usermod ipsa; do \
...
populate: tools/txfs_write
	...
	tools/txfs_write build/disk.img build/user/bin/ipsa.elf /BSM/SystemT/ipsa.elf
